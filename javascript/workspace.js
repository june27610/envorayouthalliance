...
document.addEventListener('DOMContentLoaded', function () {
  // Require sign-in for workspace + show username
  try {
    const signedIn = localStorage.getItem('isSignedIn') === 'true';
    const currentUser = JSON.parse(localStorage.getItem('envoraCurrentUser') || 'null');
    if (!signedIn || !currentUser) {
      alert('Please sign in to access the workspace.');
      window.location.href = 'signin.html?mode=signin';
      return;
    }
    const name = currentUser.username || currentUser.fullname || currentUser.email || 'User';
    const elName = document.getElementById('welcomeUsername');
